...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			log.Fatalf("Please check your TG_BOT_TOKEN env variable %s", err)
			return
		}
		...
		kbot.Handle(telebot.OnText, func(m telebot.Context) error {
			...
			validCommands := map[string]string{
				"hello": "Hello %s!, I am kbot DDDD! my version is %s",
			...
			greatingsArray := strings.Split("thanx,thx,ok,thank you,by,poka,dyakuyu,thanks,thank,Дякую,Пока,ni", ",")
